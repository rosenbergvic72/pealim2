...
import VerbCard5 from './VerbCard5Fr';
...
import StatModal5Fr from './StatModal5Fr';
...
FirebaseAnalytics.logFirebaseEvent('exercise4fr',{
screen:'exercise4fr'
});
...
const handleSpeakerPress = (mp3) => {
  ...
  const handleExerciseCompletion = async () => {
    if (!statisticsUpdated) {
      ...
      setTimeout(async () => {
        try {
          await updateStatistics('exercise5Fr', currentScore);
  ...
  onToggleDontShowAgain={handleToggleDontShowAgain5}
      ...
      <Animated.Text style={[styles.title, { opacity: fadeAnim }]}maxFontSizeMultiplier={1.2}>SÉLECTIONNEZ LA TRADUCTION</Animated.Text>
