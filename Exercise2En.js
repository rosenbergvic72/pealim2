...
const findVerbDetailsIndex = (exerciseVerb) => {
  ...
  const targetTranslation = normalize(exerciseVerb.verbEnglish);
  ...
  if (targetTranslation) {
    const exact = candidates.find(({ verb }) => {
      ...
      const correctTranslation =
        verb?.translationOptionsEn?.[correctIndex] ||
        verb?.translationOptionsEn?.[0] ||
        '';
...
FirebaseAnalytics.logFirebaseEvent('exercise2en',{
screen:'exercise2en'
});
...
exercise2LoggedRef.current=false;
  ...
  if (verbIndex < 0) {
    console.warn(
      '[Exercise2En] VerbDetails: verb not found',
      currentVerb
    );
    return;
  }
