...
import StatModal4 from './StatModal4';
...
FirebaseAnalytics.logFirebaseEvent('exercise7ru',{
screen:'exercise7ru'
});
...
useEffect(() => {
  ...
  /* ===================== ✅ Highlight helpers (как в Exercise6) ===================== */
  ...
  const renderHebrewText = (hebrewtext, formIndex, infinitive) => {
    ...
    // ✅ 17: без суффикса (иначе נ выпадет)
    if (virtualPos === 17)
      return applyToVerbWord(raw, (w) =>
  ...
  useEffect(() => {
    if (verbsData && verbsData.length > 0) {
      ...
      const withIndex = shuffledData.map(item => {
        ...
        // стараемся найти точную форму по mp3 (самый стабильный ключ)
        let idx = allForms.findIndex(v => String(v.mp3 || '') === String(item.mp3 || ''));

        // fallback если mp3 вдруг пустой
        if (idx < 0) {
  ...
  /* ===================== Mix hebrew side (keep formIndex with hebrewtext) ===================== */
  useEffect(() => {
    const start = page * 6;
    const end = start + 6;
  ...
  const handleSelectHebrew = (index) => {
    ...
    const selectedPairHeb = displayPairs[index];
    const isCorrect =
      selectedRussian !== null &&
      selectedPairHeb.hebrewtext === displayPairs[selectedRussian].correct;
        ...
        <View style={styles.verbContainerWrapper}>
          {!isButtonActive ? (
            <View style={[styles.verbContainer, styles.inactiveButton]}>
              <Text style={styles.verbTextInactive} maxFontSizeMultiplier={1.2}>ПОКАЗАТЬ ИНФИНИТИВ</Text>
            </View>
          ) : (
            !showVerb ? (
              <TouchableOpacity
              ...
              >
                <Text style={styles.verbTextActive} maxFontSizeMultiplier={1.2}>ПОКАЗАТЬ ИНФИНИТИВ</Text>
              </TouchableOpacity>
            ) : (
              <Animated.View style={[styles.verbContainer, { opacity: fadeAnim }]}>
                ...
                <Text style={styles.verbTextRu} maxFontSizeMultiplier={1.2}> {selectedPair.russian}</Text>
...
translitText: {
  ...
  // ✅ colors (как у тебя было в других упражнениях)
