...
const Exercise6 = () => {
  ...
  useEffect(() => {
    if (verbsData && verbsData.length > 0) {
      ...
      // ✅ ВАЖНО: копируем объекты, чтобы модалка не могла мутировать исходные ссылки
  ...
  useEffect(() => {
    ...
    const hebrewTexts = shuffleArrayLocal(
      currentPairs.map(pair => ({ text: pair.hebrewtext, translit: pair.translit }))
    );
    ...
    <>
      {isVerbListVisible && (
        <VerbListModal2
          ...
          language={language}
          // ✅ ВАЖНО: копии на вход модалке (вторая защита)
