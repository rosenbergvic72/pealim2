...
FirebaseAnalytics.logFirebaseEvent('exercise5am',{
screen:'exercise5am'
});
...
exercise6LoggedRef.current=false;
  ...
  const getGrade = (percentage) => {
    if (percentage === 100) return 'እጅግ አስደናቂ! ፍጹም ስኬት! ስህተት አላደረግህም!';
    if (percentage >= 90) return 'በጣም ጥሩ! ማለት ይቻላል ፍጹም ነው — ቀጥል!';
    if (percentage >= 80) return 'እጅግ ጥሩ! በጣም ጥሩ እየሰራህ ነው!';
    if (percentage >= 70) return 'ጥሩ! ብዙውን ተማርከዋል!';
    if (percentage >= 60) return 'መልካም! የተረጋጋ እድገት አለ!';
    if (percentage >= 50) return 'መጥሩ! ግን ለማሻሻል ቦታ አለ።';
    if (percentage >= 40) return 'በመጠኑ ጥሩ! ተጨማሪ ልምምድ ያስፈልጋል!';
    if (percentage >= 30) return 'መጀመር ጀምረሃል — ቀጥል!';
    if (percentage >= 20) return 'የመማር ዘዴህን ትንሽ ቀይር — ይረዳል!';
    if (percentage >= 10) return 'ከባድ ነው፣ ግን አትተው! ቀጥል!';
    return 'ብዙ ስራ ያስፈልጋል! አትተው እና ቀጥል በመማር።';
  };
...
const finishExerciseWithDelay = useCallback((finalScore) => {
  ...
  setTimeout(async () => {
    ...
    await updateStatistics('exercise6Am', Number(finalScore));
  }, 1400);
}, []);
  ...
  const getButtonStyle = (index, type) => {
    ...
    if (correctAnswers.has(index) && type === 'am') style.push(styles.deactivatedButton);
    if (type === 'am' && index === selectedRussian) style.push(styles.selectedButton);
  ...
  const resetExercise = () => {
    ...
    setCurrentVerb({
      ...
      amharic: verbConjugations[0]?.amharic || '',
  ...
  const handleStartExercise = ({ selectedTenses: nextSelectedTenses, forms, totalForms } = {}) => {
    ...
    if (verbConjugations.length > 0) {
      ...
      setCurrentVerb({
        infinitive: chosenForms[0]?.infinitive || '',
        transliteration: chosenForms[0]?.transliteration || '',
        amharic: chosenForms[0]?.amharic || '',
        audioFile: chosenForms[0]?.audioFile || '',
      });

      setMainVerb(chosenForms[0]);
      setPairs(verbConjugations);
      setRemainingPairs(chosenForms.length);
      setTotalExercises(chosenForms.length);
      setCurrentIndex(0);
      setCorrectCount(0);
      setIncorrectCount(0);
      setProgress(0);
