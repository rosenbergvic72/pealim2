...
const SearchModalPt = ({ visible, onToggle, onSelectVerb }) => {
  ...
  const handleSearch = () => {
    ...
    const results = verbsData.filter(verb => 
      ...
      verb.portu.toLowerCase().includes(lowerCaseQuery) ||
      verb.pttext.toLowerCase().includes(lowerCaseQuery) ||
    ...
    >
      <View style={styles.centeredView}>
        <View style={styles.modalView}>
          <Text style={styles.modalText}maxFontSizeMultiplier={1.2}>Pesquisar por exercício</Text>
          <TextInput
            style={styles.input}
          ...
          >
            <Text style={styles.buttonText}maxFontSizeMultiplier={1.2}>Pesquisa</Text>
          </TouchableOpacity>
          <ScrollView>
            {searchResults.length > 0 && (
              <Text style={styles.hintText}maxFontSizeMultiplier={1.2}>Clique para conjugar</Text>
            )}
            {searchResults.map((result, index) => (
              <TouchableOpacity key={index} style={styles.resultContainer} onPress={() => handleResultPress(result)}>
                ...
                <Text style={[styles.resultText1, { color: '#1C3F60' }]}maxFontSizeMultiplier={1.2}>{result.portu}</Text>
                ...
                <Text style={[styles.resultText1, { color: '#1C3F60' }]}maxFontSizeMultiplier={1.2}>{result.pttext}</Text>
          ...
          <TouchableOpacity
            ...
            onPress={onToggle}
          >
            <Text style={styles.closeButtonText}maxFontSizeMultiplier={1.2}>Fechar</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
