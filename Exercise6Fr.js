  ...
  const [language, setLanguage] = useState('fr'); // по умолчанию
    ...
    const [mainVerb, setMainVerb] = useState(null);
        ...
        const initializeVerbList = (lang, mainVerb, setVerbListForModal) => {
          ...
          const langKey = langMap[lang] || 'french';
...
useEffect(() => {
  ...
  const getButtonStyle = (index, type) => {
    ...
    if (correctAnswers.has(index) && type === 'french') {
      style.push(styles.deactivatedButton);
    }
    if (type === 'french' && index === selectedRussian) {
  ...
  onToggleDontShowAgain={handleToggleDontShowAgain6}
      ...
      <Animated.View style={[styles.progressContainer, { opacity: fadeAnim }]}>
        <View style={styles.textContainer}>
          <Text style={styles.prtext}maxFontSizeMultiplier={1.2}>CORRECT: {correctCount / 2}</Text>
      ...
      <Animated.Text style={[styles.title, { opacity: fadeAnim }]}maxFontSizeMultiplier={1.2}>CONJUGUER LE VERBE</Animated.Text>
