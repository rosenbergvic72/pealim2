...
import StatModal6Fr from './StatModal6Fr';
...
const Exercise6Fr = () => {
  ...
  const [selectedRussian, setSelectedRussian] = useState(null); // (оставлено как в RU — логика та же)
...
;
  ...
  useEffect(() => {
    const initialize = async () => {
      ...
      const hidden = await AsyncStorage.getItem('exercise6Fr_description_hidden');
      setLanguage(lang || 'fr');
  ...
  useEffect(() => {
    const checkFlagAndLang = async () => {
      ...
      if (lang) {
        ...
        if (hidden !== 'true') {
          setTimeout(() => {
            console.log('📢 Afficher la modale après chargement de la langue');
            setDescriptionModalVisible(true);
          }, 100);
        }
      }
  ...
  const handleToggleDontShowAgain6 = async () => {
    ...
    await AsyncStorage.setItem('exercise6Fr_description_hidden', newValue ? 'true' : '');
    console.log('📌 Toggle checkbox. Was:', dontShowAgain6, 'Now:', !dontShowAgain6);
  };

  const getGrade = (percentage) => {
    if (percentage === 100) return 'Exceptionnel ! Parfait ! Aucune erreur !';
    if (percentage >= 90) return 'Magnifique ! Presque parfait, continue comme ça !';
    if (percentage >= 80) return 'Excellent ! Tu t’en sors très bien !';
    if (percentage >= 70) return 'Bien ! Tu assimiles bien le contenu !';
    if (percentage >= 60) return 'Assez bien ! Des progrès stables !';
    if (percentage >= 50) return 'Pas mal ! Mais tu peux encore t’améliorer.';
    if (percentage >= 40) return 'Satisfaisant ! Continue, tu vas y arriver !';
    if (percentage >= 30) return 'Tu commences à comprendre l’essentiel, continue !';
    if (percentage >= 20) return 'Essaie de changer ta stratégie d’apprentissage, ça peut aider !';
    if (percentage >= 10) return 'C’est difficile, mais ne lâche pas ! Continue à pratiquer.';
    return 'Il faut travailler sérieusement ! Ne te décourage pas et continue à apprendre.';
  };
...
const isNifalForCurrentVerb = useMemo(() => {
  ...
  /* ====================================================================== */
  /* ✅ Подсветка частей глагола на иврите (как EN/AM) — ДО МИКСА            */
  /* ====================================================================== */
...
if (virtualPos === 23 || virtualPos === 24)
  ...
  useEffect(() => {
    if (verbsData && verbsData.length > 0) {
      ...
      // ✅ formIndex проставляем ДО перемешивания (важно для подсветки)
  ...
  useEffect(() => {
    ...
    // ✅ миксуем вправо, но переносим formIndex → hebrewFormIndex
    const hebrewItems = shuffleArrayLocal(
      currentPairs.map(pair => ({
        text: pair.hebrewtext,
        translit: pair.translit,
        hebrewFormIndex: pair.formIndex, // ← важно
      }))
    );
    ...
    const mixedPairs = currentPairs.map((pair, index) => ({
      ...pair,
      // слева — французский
      frtext: pair.frtext ?? pair.frenchtext ?? pair.russiantext,
      hebrewtext: hebrewItems[index].text,
      translit: hebrewItems[index].translit,
      hebrewFormIndex: hebrewItems[index].hebrewFormIndex, // ← важно
  ...
  const getButtonStyle = (index, type) => {
    ...
    if (correctAnswers.has(index) && type === 'french') style.push(styles.deactivatedButton);
    if (type === 'french' && index === selectedRussian) style.push(styles.selectedButton);
  ...
  const resetExercise = () => {
    ...
    // ✅ formIndex снова ДО shuffle
    const verbConjugations = verbConjugationsRaw.map((v, i) => ({ ...v, formIndex: i + 1 }));
  ...
  const handleSelectVerb = (verb) => {
    ...
    // ✅ formIndex ДО любого перемешивания/передачи в модалки
  ...
  />
            ...
            <Animated.View style={[styles.progressContainer, { opacity: fadeAnim }]}>
              <View style={styles.textContainer}>
                <Text style={styles.prtext} maxFontSizeMultiplier={1.2}>JUSTE: {correctCount / 2}</Text>
                <Text style={styles.prtext} maxFontSizeMultiplier={1.2}>FAUX: {incorrectCount}</Text>
              </View>
            ...
            {currentVerb && (
              <Animated.View style={[styles.verbContainer, { opacity: fadeAnim }]}>
                ...
                <Text style={styles.verbTextRu} maxFontSizeMultiplier={1.2}> {currentVerb.french}</Text>
            ...
            <Animated.View style={{ transform: [{ translateY: blockAnimation }] }}>
              {displayPairs.map((pair, index) => (
                <View key={index} style={styles.row}>
                  <TouchableOpacity
                    style={getButtonStyle(index, 'french')}
                  ...
                  >
                    <Text
                      ...
                      maxFontSizeMultiplier={1.2}
                    >
                      {/* ✅ подсветка как в EN/AM */}
                      {renderHebrewText(pair.hebrewtext, pair.hebrewFormIndex)}
                    </Text>

                    {/* держим высоту стабильной: скрываем только opacity */}
                    <Text
  ...
  {pair.translit}
            ...
            {exerciseCompleted && (
              <CompletionMessageFr
                ...
                navigateToMenu={() => navigation.navigate('MenuFr')}
