...
const SearchModalEs = ({ visible, onToggle, onSelectVerb }) => {
  ...
  const handleSearch = () => {
    ...
    const results = verbsData.filter(verb => 
      ...
      verb.spanish.toLowerCase().includes(lowerCaseQuery) ||
      verb.estext.toLowerCase().includes(lowerCaseQuery) ||
    ...
    >
      <View style={styles.centeredView}>
        <View style={styles.modalView}>
          <Text style={styles.modalText}maxFontSizeMultiplier={1.2}>Búsqueda por ejercicio</Text>
          <TextInput
            style={styles.input}
          ...
          >
            <Text style={styles.buttonText}maxFontSizeMultiplier={1.2}>Buscar</Text>
          </TouchableOpacity>
          <ScrollView>
            {searchResults.length > 0 && (
              <Text style={styles.hintText}maxFontSizeMultiplier={1.2}>Haga clic para conjugar</Text>
            )}
            {searchResults.map((result, index) => (
              <TouchableOpacity key={index} style={styles.resultContainer} onPress={() => handleResultPress(result)}>
                ...
                <Text style={[styles.resultText1, { color: '#1C3F60' }]}maxFontSizeMultiplier={1.2}>{result.spanish}</Text>
                <Text style={[styles.resultText, { color: '#FF5733' }]}maxFontSizeMultiplier={1.2}>{result.transliteration}</Text>
                <Text style={[styles.resultText1, { color: '#1C3F60' }]}maxFontSizeMultiplier={1.2}>{result.estext}</Text>
          ...
          <TouchableOpacity
            ...
            onPress={onToggle}
          >
            <Text style={styles.closeButtonText}maxFontSizeMultiplier={1.2}>CERRAR</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
