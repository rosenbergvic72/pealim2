...
*/
const PARTNER_REDEEM_URL = ''; // ← поставь свой URL, например https://your-domain.com/api/redeem-code
...
async function redeemPartnerCodeOnServer(code) {
...
const STR = {
  english: {
    ...
    redeemSuccessBody: 'Code activated. Now tap “Restore access”.',
  ...
  русский: {
    ...
    redeemSuccessBody: 'Код активирован. Теперь нажмите «Восстановить доступ».',
  ...
  français: {
    ...
    redeemSuccessBody: 'Code activé. Appuyez maintenant sur « Restaurer l’accès ».',
  ...
  español: {
    ...
    redeemSuccessBody: 'Código activado. Ahora pulsa «Restaurar acceso».',
  ...
  português: {
    ...
    redeemSuccessBody: 'Código ativado. Agora toque em «Restaurar acesso».',
  ...
  'አማርኛ': {
    ...
    redeemSuccessBody: 'ኮድ ተነቅቷል። አሁን «መዳረሻ መመለስ» ይጫኑ።',
    choosePlan: 'እቅድ ይምረጡ',
  ...
  العربية: {
    ...
    redeemSuccessBody: 'تم تفعيل الرمز. الآن اضغط «استعادة الوصول».',
    choosePlan: 'اختر خطة',
...
export default function Paywall({ navigation }) {
  ...
  // ✅ Реальная активация кода (UI готов, сервер подключишь позже)
  const submitRedeemCode = async () => {
    ...
    try {
      const r = await redeemPartnerCodeOnServer(normalized);
      ...
      // Переоткрываем restore вручную по кнопке пользователя (на всякий случай)
      // (ничего в IAP-сервер не трогаем)
    } catch (e) {
      setRedeemOK(false);
