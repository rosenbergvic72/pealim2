...
useEffect(() => {
  ...
  //    - уникальные по pttext
  const poolMap = new Map(); // key = pttext.trim()

  for (const v of sameInfinitiveVerbs) {
    if (!v.pttext) continue;

    if (v.pttext === currentVerb.pttext) continue;         // тот же португальский текст
    if (v.hebrewtext === currentVerb.hebrewtext) continue; // та же форма на иврите

    const key = v.pttext.trim();
    if (!poolMap.has(key)) {
  ...
  // сколько неправильных с тем же gender нужно (учитывая, что правильный уже один)
  ...
  // 3) Правильный + неправильные и защита от дублей по pttext
  const allCandidates = [
