...
const Exercise4Ar = () => {
  ...
  const renderHebrewText = (hebrewtext, formIndex, infinitive) => {
    ...
    if (virtualPos === 30)
      return applyToVerbWord(raw, w => renderWithColorRules(w, { prefix: 'ت' })); // (оставляем как было? нет — в иврите "ת")
    // ⚠️ исправление: выше должна быть 'ת', а не арабская 'ت' (оставляем правильную):
    // но эта ветка вообще про иврит, так что:
    // return applyToVerbWord(raw, w => renderWithColorRules(w, { prefix: 'ת' }));
  ...
  const getButtonStyle = (index, type) => {
    ...
    if (correctAnswers.has(index) && type === 'arabic') style.push(styles.deactivatedButton);
    if (type === 'arabic' && index === selectedRussian) style.push(styles.selectedButton);
        ...
        <View style={styles.verbContainerWrapper}>
          {!isButtonActive ? (
            <View style={[styles.verbContainer, styles.inactiveButton]}>
              <Text style={styles.verbTextInactive} maxFontSizeMultiplier={1.2}>إظهار المصدر</Text>
            </View>
          ) : (
            !showVerb ? (
              <TouchableOpacity
              ...
              >
                <Text style={styles.verbTextActive} maxFontSizeMultiplier={1.2}>إظهار المصدر</Text>
              </TouchableOpacity>
            ) : (
