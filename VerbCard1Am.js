...
const VerbCard1Am = ({ verbData, onAnswer, soundEnabled }) => {
  ...
  return (
    <Animated.View style={[styles.cardContainer, shadowEnabled && styles.cardShadow]}>
      <View style={styles.hebrewVerbContainer}>
        ...
        <Animated.Text style={[styles.root, { opacity: opacity3 }]} maxFontSizeMultiplier={1.2}>{`ሥር: ${verbData.root}`}</Animated.Text>
        <Animated.Text style={[styles.bin, { opacity: opacity4 }]} maxFontSizeMultiplier={1.2}>{`ቢንያን: ${verbData.binyan}`}</Animated.Text>
