...
import CompletionMessageEn from './CompletionMessageAm';
...
useEffect(() => {
  ...
  const generateOptions = (verbData) => {
    if (!verbData || !verbData.translationOptionsAm || !Number.isInteger(verbData.correctTranslationIndex)) {
...
const playAudio = async (audioFile) => {
  ...
  const handleGenderToggle = () => {
    setIsGenderMan((prev) => {
        ...
        updateVerbDetails(shuffledVerbs[currentIndex], newIsGenderMan, verbDetails.amtext !== '');
    ...
    {!isVerbListVisible && (
      <ScrollView contentContainerStyle={styles.scrollViewContent}>
        <View style={styles.container}>
          ...
          >
            <Text style={styles.nextButtonText}maxFontSizeMultiplier={1.1}>ቀጣዩ ግስ</Text>
          </TouchableOpacity>
