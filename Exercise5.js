...
import StatModal5 from './StatModal5';
...
FirebaseAnalytics.logFirebaseEvent('exercise4ru',{
screen:'exercise4ru'
});
...
const handleSpeakerPress = (mp3) => {
  ...
  const handleExerciseCompletion = async () => {
    if (!statisticsUpdated) {
      ...
      setTimeout(async () => {
        try {
          await updateStatistics('exercise5', currentScore);
  ...
  onToggleDontShowAgain={handleToggleDontShowAgain5}
      ...
      <Animated.Text style={[styles.title, { opacity: fadeAnim }]}maxFontSizeMultiplier={1.2}>ВЫБЕРИ ПЕРЕВОД</Animated.Text>
