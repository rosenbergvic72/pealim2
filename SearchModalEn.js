...
const SearchModalEn = ({ visible, onToggle, onSelectVerb }) => {
  ...
  const handleSearch = () => {
    ...
    const results = verbsData.filter(verb => 
      ...
      verb.english.toLowerCase().includes(lowerCaseQuery) ||
      verb.entext.toLowerCase().includes(lowerCaseQuery) ||
    ...
    >
      <View style={styles.centeredView}>
        <View style={styles.modalView}>
          <Text style={styles.modalText}maxFontSizeMultiplier={1.2}>Search by exercise</Text>
          <TextInput
            style={styles.input}
          ...
          >
            <Text style={styles.buttonText}maxFontSizeMultiplier={1.2}>Search</Text>
          </TouchableOpacity>
          <ScrollView>
            {searchResults.length > 0 && (
              <Text style={styles.hintText}maxFontSizeMultiplier={1.2}>Click to conjugate</Text>
            )}
            {searchResults.map((result, index) => (
              <TouchableOpacity key={index} style={styles.resultContainer} onPress={() => handleResultPress(result)}>
                ...
                <Text style={[styles.resultText1, { color: '#1C3F60' }]}maxFontSizeMultiplier={1.2}>{result.english}</Text>
                <Text style={[styles.resultText, { color: '#FF5733' }]}maxFontSizeMultiplier={1.2}>{result.transliteration}</Text>
                <Text style={[styles.resultText1, { color: '#1C3F60' }]}maxFontSizeMultiplier={1.2}>{result.entext}</Text>
