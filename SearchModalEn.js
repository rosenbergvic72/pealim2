...
const SearchModalEn = ({ visible, onToggle, onSelectVerb }) => {
  ...
  const pickEnglishInf = useCallback((row) => {
    return (
      row?.english ||
      row?.verbEnglish ||
      row?.englishInfinitive ||
      row?.en ||
      row?.enTextShort ||
      row?.entext || // fallback
      '—'
  ...
  const allVerbsList = useMemo(() => {
    ...
    for (const row of verbsData6 || []) {
      ...
      if (!groups.has(key)) {
        groups.set(key, {
          ...
          englishInfinitive: pickEnglishInf(row),
          sampleRow: row,
        });
      }
    }
    ...
    return list;
  }, [normalizeHebrew, pickEnglishInf, binyanByInfinitiveHebrew]);
  ...
  const handleSearch = useCallback(() => {
    ...
    const results = verbsData6.filter((v) =>
      ...
      (v.entext || '').toLowerCase().includes(q) ||
    ...
    <Modal animationType="slide" transparent visible={visible} onRequestClose={onToggle}>
      <View style={styles.centeredView}>
        <View style={styles.modalView}>
          <Text style={styles.modalText}>Exercise verbs</Text>

          {/* Search */}
          <TextInput
            style={styles.input}
            // placeholder="Enter a word to search (min. 3 chars)"
            value={searchQuery}
          ...
          >
                ...
                {searchResults.length === 0 ? (
                  ...
                  <>
                    <Text style={styles.hintText}>
                      Tap a result to open conjugation
                    </Text>
            ...
            <Text style={styles.sectionTitle}>Choose a verb to practice</Text>
            ...
            {allVerbsList.map((item, idx) => (
              ...
              >
                ...
                <Text style={[styles.tableCell, styles.colEn]}>
                  {item.englishInfinitive}
                </Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  englishShortText: { fontWeight: '600', color: '#1C3F60' },
  ...
  englishPhraseText: { color: '#0B7285', textAlign: 'right' },
