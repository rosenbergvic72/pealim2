...
import TaskDescriptionModal1 from './TaskDescriptionModal1Pt';
...
const Exercise1Pt = ({ navigation }) => {
  ...
  const generateOptions = (verbData) => {
    ...
    const correctTranslation = verbData.translationOptionsPt[correctAnswerIndex];
    const incorrectOptions = verbData.translationOptionsPt.filter(
  ...
  const handleExerciseCompletion = async () => {
    if (!statisticsUpdated) {
      ...
      setTimeout(async () => {
        try {
          await updateStatistics('exercise1Pt', currentScore);
        } catch (error) {
  ...
  const updateVerbDetails = (currentVerb, isGenderMan, showRussianText = false) => {
    ...
    if (matchedVerbs.length > 0) {
      ...
      if (selectedVerb) {
        ...
        setVerbDetails({ hebrewtext: 'Глагол не найден', translit: '', pttext: '', mp3: '' });
      }
    } else {
      setVerbDetails({ hebrewtext: 'Глагол не найден', translit: '', pttext: '', mp3: '' });
    }
...
const playAudio = async (audioFile) => {
  ...
  const handleGenderToggle = () => {
    setIsGenderMan((prev) => {
        ...
        updateVerbDetails(shuffledVerbs[currentIndex], newIsGenderMan, verbDetails.pttext !== '');
  ...
  return (
    <ScrollView contentContainerStyle={styles.scrollViewContent}>
      ...
      <Animated.Text style={[styles.title, { opacity: fadeAnim }]}maxFontSizeMultiplier={1.2}>SELECIONAR TRADUÇÃO</Animated.Text>
...
<VerbDetailsContainer
  ...
  showRussianText={verbDetails.pttext !== ''}
...
>
  {optionsOrder.map((option, index) => (
    ...
    >
      ...
      >
        <Text style={styles.nextButtonText}maxFontSizeMultiplier={1.1}>PRÓXIMO VERBO</Text>
      </TouchableOpacity>
