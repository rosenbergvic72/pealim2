...
import StatModal7Es from './StatModal7Es';
...
FirebaseAnalytics.logFirebaseEvent('exercise8es',{
screen:'exercise8es'
});
...
const [isDescriptionModalVisible, setDescriptionModalVisible] = useState(false);
    ...
    const [language, setLanguage] = useState('es'); // ← по умолчанию ru
...
useEffect(() => {
  ...
  //       verbs
  //         .filter((verb) => verb.estext !== currentVerb.estext)
  //         .map((verb) => ({
  //           estext: verb.estext,
  //           gender: verb.gender,
  //         }))
  //     ).slice(0, 5);
  //     const answers = shuffleArray([{ estext: currentVerb.estext, gender: currentVerb.gender }, ...incorrectAnswers]);

  //     setDisplayPairs(
  //       answers.map((answer) => ({
  //         ...currentVerb,
  //         estext: answer.estext,
...
useEffect(() => {
  if (verbs.length > 0) {
    ...
    const makeKey = (v) => `${String(v?.estext || '').trim()}__${String(v?.gender || '').trim()}`;
    const uniqByKey = (arr) => {
      const seen = new Set();
      const out = [];
      for (const v of arr) {
    ...
    const basePoolAll = uniqByKey(
      verbsData.filter((v) => v.estext !== currentVerb.estext)
    );
    ...
    const answers = shuffleArray([
      ...
      ...picked.map((v) => ({ estext: v.estext, gender: v.gender })),
    ]);
      ...
      <View style={styles.topBar}>
        ...
        <View style={styles.buttonContainer}>
          ...
          <TouchableOpacity onPress={handleButton3Press}>
            ...
            <StatModal7Es visible={isStatModalVisible} onToggle={() => setIsStatModalVisible(false)} statistics={statistics} />
          </TouchableOpacity>
        ...
        <>
          ...
          {!showTranslation ? (
            <TouchableOpacity style={styles.translationButton} onPress={() => setShowTranslation(true)}>
              <Text style={styles.verbTextActive}maxFontSizeMultiplier={1.2}>TRADUCCIÓN</Text>
            </TouchableOpacity>
          ) : (
