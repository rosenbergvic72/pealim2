...
const StatModal4Ar = ({ visible, onToggle }) => {
  ...
  useEffect(() => {
    if (visible) {
      const fetchStats = async () => {
        const stats = await getStatistics('exercise4Ar');
      ...
      <View style={styles.centeredView}>
        <View style={styles.modalView} ref={viewRef}>
          ...
          <Animated.View style={{ opacity: animationFinished ? contentOpacity : 0 }}>
            ...
            <Text style={styles.modalText}maxFontSizeMultiplier={1.2}>إحصائيات التمرين 7</Text>
            {stats ? (
              <View style={styles.statsContainer}>
                <View style={styles.row}>
                  <Text style={styles.label}maxFontSizeMultiplier={1.2}>عدد المرات المكتملة</Text>
                  <Text style={styles.number}maxFontSizeMultiplier={1.2}>{stats.timesCompleted}</Text>
                </View>
                <View style={styles.row}>
                  <Text style={styles.label}maxFontSizeMultiplier={1.2}>أفضل نتيجة, %</Text>
                  <Text style={styles.number}maxFontSizeMultiplier={1.2}>{stats && stats.bestScore ? parseFloat(stats.bestScore).toFixed(2) : 'Н/Д'}</Text>
                </View>
                <View style={styles.row}>
                  <Text style={styles.label}maxFontSizeMultiplier={1.2}>متوسط النتيجة, %</Text>
                  <Text style={styles.number}maxFontSizeMultiplier={1.2}>{stats.averageCompletionRate.toFixed(2)}</Text>
                </View>
              </View>
            ) : (
              <Text style={styles.statsText}maxFontSizeMultiplier={1.2}>لم يتم العثور على إحصائيات</Text>
            )}
            <TouchableOpacity
