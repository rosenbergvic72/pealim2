...
import StatModal7Am from './StatModal7Am';
...
const Exercise7Am = () => {
  ...
  const [isDescriptionModalVisible, setDescriptionModalVisible] = useState(false);
      ...
      const [language, setLanguage] = useState('am'); // ← по умолчанию ru
...
useEffect(() => {
  ...
  //       verbs
  //         .filter((verb) => verb.amtext !== currentVerb.amtext)
  //         .map((verb) => ({
  //           amtext: verb.amtext,
  //           gender: verb.gender,
  //         }))
...
useEffect(() => {
  if (verbs.length > 0) {
    ...
    const makeKey = (v) => `${String(v?.amtext || '').trim()}__${String(v?.gender || '').trim()}`;
    const uniqByKey = (arr) => {
      const seen = new Set();
      const out = [];
      for (const v of arr) {
    ...
    const basePoolAll = uniqByKey(
      verbsData.filter((v) => v.amtext !== currentVerb.amtext)
    );
    ...
    const answers = shuffleArray([
      { amtext: currentVerb.amtext, gender: currentVerb.gender },
      ...picked.map((v) => ({ amtext: v.amtext, gender: v.gender })),
    ]);
  ...
  const handleAnswer = (index) => {
    ...
    if (selectedAnswer.amtext === verbs[currentIndex].amtext) {
  ...
  const getButtonStyle = (index) => {
    ...
    if (selectedAnswer === index) {
      if (displayPairs[index].amtext === verbs[currentIndex].amtext) {
      ...
      <View style={styles.topBar}>
        ...
        <View style={styles.buttonContainer}>
          ...
          <TouchableOpacity onPress={handleButton3Press}>
            ...
            <StatModal7Am visible={isStatModalVisible} onToggle={() => setIsStatModalVisible(false)} statistics={statistics} />
          </TouchableOpacity>
        ...
        <>
          ...
          {!showTranslation ? (
            <TouchableOpacity style={styles.translationButton} onPress={() => setShowTranslation(true)}>
              <Text style={styles.verbTextActive}maxFontSizeMultiplier={1.2}>ትርጉም</Text>
            </TouchableOpacity>
          ) : (
