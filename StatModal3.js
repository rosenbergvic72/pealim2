...
const StatModal3 = ({ visible, onToggle }) => {
  ...
  useEffect(() => {
    if (visible) {
      const fetchStats = async () => {
        const exerciseId = 'exercise3'; // Ensure you use the correct exercise ID
  ...
  const handleShare = async () => {
    try {
      ...
      await Sharing.shareAsync(uri, {
        dialogTitle: 'Статистика задания',
        mimeType: 'image/png',
      });
    } catch (error) {
      ...
      <View style={styles.centeredView}>
        <View style={styles.modalView} ref={viewRef}>
          ...
          <Animated.View style={{ opacity: animationFinished ? contentOpacity : 0 }}>
            ...
            <Text style={styles.modalText}maxFontSizeMultiplier={1.2}>СТАТИСТИКА УПРАЖНЕНИЯ 3</Text>
            {stats ? (
              <View style={styles.statsContainer}>
                <View style={styles.row}>
                  <Text style={styles.label}maxFontSizeMultiplier={1.2}>ВЫПОЛНЕНО РАЗ</Text>
                  <Text style={styles.number}>{stats.timesCompleted}</Text>
                </View>
                <View style={styles.row}>
                  <Text style={styles.label}maxFontSizeMultiplier={1.2}>ЛУЧШИЙ РЕЗУЛЬТАТ, %</Text>
                  <Text style={styles.number}maxFontSizeMultiplier={1.2}>{stats && stats.bestScore ? parseFloat(stats.bestScore).toFixed(2) : 'Н/Д'}</Text>
                </View>
                <View style={styles.row}>
                  <Text style={styles.label}maxFontSizeMultiplier={1.2}>СРЕДНИЙ РЕЗУЛЬТАТ, %</Text>
                  <Text style={styles.number}maxFontSizeMultiplier={1.2}>{stats.averageCompletionRate.toFixed(2)}</Text>
                </View>
              </View>
            ) : (
              <Text style={styles.statsText}maxFontSizeMultiplier={1.2}>Статистика не найдена.</Text>
            )}
            <TouchableOpacity
