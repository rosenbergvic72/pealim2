...
useEffect(() => {
  ...
  const sameInfinitiveVerbs = verbs.filter(
    verb => verb.infinitive === currentVerb.infinitive
  );
  ...
  //    - уникальные по amtext
  const poolMap = new Map(); // key = amtext.trim()

  for (const v of sameInfinitiveVerbs) {
    if (!v.amtext) continue;

    if (v.amtext === currentVerb.amtext) continue;          // тот же амхарский текст
    if (v.hebrewtext === currentVerb.hebrewtext) continue;  // та же форма на иврите

    const key = v.amtext.trim();
    if (!poolMap.has(key)) {
  ...
  // 3) Правильный + неправильные и ещё одна защита от дублей по amtext
  const allCandidates = [
