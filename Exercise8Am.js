...
const HEBREW_LINE_H = Platform.OS === 'android' ? 36 : 34;
...
FirebaseAnalytics.logFirebaseEvent('exercise6am',{
screen:'exercise6am'
});
...
useEffect(() => {
  ...
  const getBinyanFromV1 = useCallback((currentV6) => {
    ...
    const meaningEn = normKey(currentV6?.amharic || currentV6?.entext || '');
    ...
    // 1) try exact meaning match (translationOptionsEn at correct index)
    for (const c of candidates) {
      const idx = Number(c?.correctTranslationIndex);
      const opt = normKey(
        (Array.isArray(c?.translationOptionsEn) ? c.translationOptionsEn[idx] : '') ||
          (Array.isArray(c?.translationOptions) ? c.translationOptions[idx] : '')
      );
      if (opt && meaningEn && opt === meaningEn) return String(c?.binyan || '');
    }
...
useEffect(() => {
  ...
  const MAX_INትክክል = 5;          // 1 правильный + до 5 неправильных
  ...
  // 1) Пул кандидатов:
  //    - есть entext
  //    - entext отличается от правильного
  //    - hebrewtext отличается от текущего (убираем совпадающие формы)
  //    - уникальные по entext
  const poolMap = new Map(); // key = entext.trim()

  for (const v of sameInfinitiveVerbs) {
    if (!v.entext) continue;

    if (v.entext === currentVerb.entext) continue;           // тот же англ. текст, что у правильного
    if (v.hebrewtext === currentVerb.hebrewtext) continue;   // та же форма на иврите (напр. תאבד)

    const key = v.entext.trim();
    if (!poolMap.has(key)) {
  ...
  // значит нужна минимум MIN_SAME_GENDER_TOTAL - 1 неправильных такого же gender
  ...
  const sameGenderIncorrect = shuffledSameGender.slice(
    0,
    Math.min(needSameGenderIncorrect, MAX_INትክክል, sameGenderPool.length)
  );

  const remainingSlots = MAX_INትክክል - sameGenderIncorrect.length;
  ...
  // 3) Правильный + неправильные и защита от дублей по entext
  const allCandidates = [
  ...
  onToggleDontShowAgain={handleToggleDontShowAgain8}
      ...
      <Animated.View style={[styles.progressContainer, { opacity: fadeAnim }]}>
        <View style={styles.textContainer}>
          ...
          <Text style={styles.prtext}maxFontSizeMultiplier={1.2}>INትክክል: {incorrectCount}</Text>
        </View>
        ...
        <Animated.View style={[styles.verbContainer, { opacity: fadeAnim }]}>
          {verbs[currentIndex] && (
            <>
              ...
              <Text style={styles.verbTextRu}maxFontSizeMultiplier={1.2}>{verbs[currentIndex].amharic}</Text>
