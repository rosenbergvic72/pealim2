...
const StatModal3Fr = ({ visible, onToggle }) => {
  ...
  useEffect(() => {
    if (visible) {
      const fetchStats = async () => {
        const exerciseId = 'exercise3Fr'; // Ensure you use the correct exercise ID
  ...
  const handleShare = async () => {
    try {
      ...
      await Sharing.shareAsync(uri, {
        dialogTitle: 'Statistiques de l\'exercice',
        mimeType: 'image/png',
      });
    } catch (error) {
      ...
      <View style={styles.centeredView}>
        <View style={styles.modalView} ref={viewRef}>
          ...
          <Animated.View style={{ opacity: animationFinished ? contentOpacity : 0 }}>
            ...
            <Text style={styles.modalText}maxFontSizeMultiplier={1.2}>Statistiques de l'exercice 3</Text>
            {stats ? (
              <View style={styles.statsContainer}>
                <View style={styles.row}>
                  <Text style={styles.label}maxFontSizeMultiplier={1.2}>NOMBRE DE FOIS TERMINÉ</Text>
                  <Text style={styles.number}>{stats.timesCompleted}</Text>
                </View>
                <View style={styles.row}>
                  <Text style={styles.label}maxFontSizeMultiplier={1.2}>MEILLEUR SCORE, %</Text>
                  <Text style={styles.number}maxFontSizeMultiplier={1.2}>{stats && stats.bestScore ? parseFloat(stats.bestScore).toFixed(2) : 'Н/Д'}</Text>
                </View>
                <View style={styles.row}>
                  <Text style={styles.label}maxFontSizeMultiplier={1.2}>SCORE MOYEN, %</Text>
                  <Text style={styles.number}maxFontSizeMultiplier={1.2}>{stats.averageCompletionRate.toFixed(2)}</Text>
                </View>
              </View>
            ) : (
              <Text style={styles.statsText}maxFontSizeMultiplier={1.2}>Aucune statistique trouvée.</Text>
            )}
            <TouchableOpacity
