...
FirebaseAnalytics.logFirebaseEvent('exercise8ru',{
screen:'exercise8ru'
});
...
useEffect(() => {
  ...
  //       verbs
  //         .filter((verb) => verb.russiantext !== currentVerb.russiantext)
  //         .map((verb) => ({
  //           russiantext: verb.russiantext,
  //           gender: verb.gender,
  //         }))
  //     ).slice(0, 5);
  ...
  //       answers.map((answer) => ({
  //         ...currentVerb,
  //         russiantext: answer.russiantext,
  ...
  if (verbs.length > 0) {
    ...
    const makeKey = (v) => `${String(v?.russiantext || '').trim()}__${String(v?.gender || '').trim()}`;
    const uniqByKey = (arr) => {
      const seen = new Set();
      const out = [];
      for (const v of arr) {
    ...
    const basePoolAll = uniqByKey(
      verbsData.filter((v) => v.russiantext !== currentVerb.russiantext)
    );
    ...
    const answers = shuffleArray([
      ...
      ...picked.map((v) => ({ russiantext: v.russiantext, gender: v.gender })),
    ]);
      ...
      <View style={styles.topBar}>
        ...
        <View style={styles.buttonContainer}>
          ...
          <TouchableOpacity onPress={handleButton3Press}>
            ...
            <StatModal7 visible={isStatModalVisible} onToggle={() => setIsStatModalVisible(false)} statistics={statistics} />
          </TouchableOpacity>
  ...
  onToggleDontShowAgain={handleToggleDontShowAgain7}
      ...
      <Animated.Text style={[styles.title, { opacity: fadeAnim }]}maxFontSizeMultiplier={1.2}>СПРЯГАЕМ ГЛАГОЛЫ</Animated.Text>
        ...
        <>
          ...
          {!showTranslation ? (
            <TouchableOpacity style={styles.translationButton} onPress={() => setShowTranslation(true)}>
              <Text style={styles.verbTextActive}maxFontSizeMultiplier={1.2}>ПЕРЕВОД</Text>
            </TouchableOpacity>
          ) : (
