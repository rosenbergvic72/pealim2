...
import StatModal7En from './StatModal7En';
...
FirebaseAnalytics.logFirebaseEvent('exercise8en',{
screen:'exercise8en'
});
...
useEffect(() => {
  if (verbs.length > 0) {
    ...
    const makeKey = (v) => `${String(v?.entext || '').trim()}__${String(v?.gender || '').trim()}`;
    const uniqByKey = (arr) => {
      const seen = new Set();
      const out = [];
      for (const v of arr) {
    ...
    const basePoolAll = uniqByKey(
      verbsData.filter((v) => v.entext !== currentVerb.entext)
    );
    ...
    const answers = shuffleArray([
      ...
      ...picked.map((v) => ({ entext: v.entext, gender: v.gender })),
    ]);
      ...
      <View style={styles.topBar}>
        ...
        <View style={styles.buttonContainer}>
          ...
          <TouchableOpacity onPress={handleButton3Press}>
            ...
            <StatModal7En visible={isStatModalVisible} onToggle={() => setIsStatModalVisible(false)} statistics={statistics} />
          </TouchableOpacity>
        ...
        <>
          ...
          {!showTranslation ? (
            <TouchableOpacity style={styles.translationButton} onPress={() => setShowTranslation(true)}>
              <Text style={styles.verbTextActive}maxFontSizeMultiplier={1.2}>TRANSLATION</Text>
            </TouchableOpacity>
          ) : (
...
const styles = StyleSheet.create({
 ...
 scrollViewContent: {
  ...
  translationButton: {
    ...
    paddingHorizontal: 5,
    borderRadius: 5,
    width: 125, // Set the same fixed width as verbTextRu
    alignItems: 'center',
  },
  audioIcon: {
    width: 26,
