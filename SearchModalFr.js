...
const SearchModalFr = ({ visible, onToggle, onSelectVerb }) => {
  ...
  const handleSearch = () => {
    ...
    const results = verbsData.filter(verb => 
      ...
      verb.french.toLowerCase().includes(lowerCaseQuery) ||
      verb.frtext.toLowerCase().includes(lowerCaseQuery) ||
    ...
    >
      <View style={styles.centeredView}>
        <View style={styles.modalView}>
          <Text style={styles.modalText}maxFontSizeMultiplier={1.2}>Recherche par exercice</Text>
          <TextInput
            style={styles.input}
          ...
          >
            <Text style={styles.buttonText}maxFontSizeMultiplier={1.2}>Recherche</Text>
          </TouchableOpacity>
          <ScrollView>
            {searchResults.length > 0 && (
              <Text style={styles.hintText}maxFontSizeMultiplier={1.2}>Cliquez pour conjuguer</Text>
            )}
            {searchResults.map((result, index) => (
              <TouchableOpacity key={index} style={styles.resultContainer} onPress={() => handleResultPress(result)}>
                ...
                <Text style={[styles.resultText1, { color: '#1C3F60' }]}maxFontSizeMultiplier={1.2}>{result.french}</Text>
                ...
                <Text style={[styles.resultText1, { color: '#1C3F60' }]}maxFontSizeMultiplier={1.2}>{result.frtext}</Text>
          ...
          <TouchableOpacity
            ...
            onPress={onToggle}
          >
            <Text style={styles.closeButtonText}maxFontSizeMultiplier={1.2}>Fermer</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
