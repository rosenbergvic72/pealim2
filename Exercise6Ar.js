...
const Exercise6Ar = () => {
 ...
 const finishExerciseWithDelay = useCallback((finalScore) => {
  ...
  setTimeout(async () => {
    ...
    await updateStatistics('exercise6Ar', Number(finalScore));
  }, 1400);
}, []);
  ...
  useEffect(() => {
    ...
    const hebrewTexts = shuffleArrayLocal(
    ...
    const mixedPairs = currentPairs.map((pair, index) => ({
      ...
      hebrewFormIndex: hebrewTexts[index].hebrewFormIndex,
  ...
  const resetExercise = () => {
    ...
    const baseForms = buildVerbForms(selectedVerb);
    const shuffledForms = shuffleArrayLocal([...baseForms]);

    setCurrentVerb({
      infinitive: baseForms[0]?.infinitive || '',
      transliteration: baseForms[0]?.transliteration || '',
      arabic: baseForms[0]?.arabic || '',
      audioFile: baseForms[0]?.audioFile || '',
    });
  ...
  const handleStartExercise = ({ selectedTenses: nextSelectedTenses, forms, totalForms } = {}) => {
    ...
    const shuffledForms = shuffleArrayLocal([...chosenForms]);
