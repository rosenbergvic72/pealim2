...
FirebaseAnalytics.logFirebaseEvent('exercise8pt',{
screen:'exercise8pt'
});
...
exercise7LoggedRef.current=false;
 ...
 useEffect(() => {
  ...
  //       verbs
  //         .filter((verb) => verb.pttext !== currentVerb.pttext)
  //         .map((verb) => ({
  //           pttext: verb.pttext,
  //           gender: verb.gender,
  //         }))
 ...
 useEffect(() => {
  if (verbs.length > 0) {
    ...
    const makeKey = (v) => `${String(v?.pttext || '').trim()}__${String(v?.gender || '').trim()}`;
    const uniqByKey = (arr) => {
      const seen = new Set();
      const out = [];
      for (const v of arr) {
    ...
    const basePoolAll = uniqByKey(
      verbsData.filter((v) => v.pttext !== currentVerb.pttext)
    );
    ...
    const answers = shuffleArray([
      { pttext: currentVerb.pttext, gender: currentVerb.gender },
      ...picked.map((v) => ({ pttext: v.pttext, gender: v.gender })),
    ]);
      ...
      <View style={styles.topBar}>
        ...
        <View style={styles.buttonContainer}>
          ...
          <TouchableOpacity onPress={handleButton3Press}>
            ...
            <StatModal7Pt visible={isStatModalVisible} onToggle={() => setIsStatModalVisible(false)} statistics={statistics} />
          </TouchableOpacity>
        ...
        <>
          ...
          {!showTranslation ? (
            <TouchableOpacity style={styles.translationButton} onPress={() => setShowTranslation(true)}>
              <Text style={styles.verbTextActive}maxFontSizeMultiplier={1.2}>TRADUÇÃO</Text>
            </TouchableOpacity>
          ) : (
